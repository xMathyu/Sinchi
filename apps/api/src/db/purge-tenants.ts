...
async function inventario(
  ...
  const r = (await db.execute(sql`
    select t.slug,
           t.name,
           (select count(*) from memberships m where m.tenant_id = t.id)  as records,
  ...
  return (r.rows ?? (r as unknown as Row[])).map((f) => ({
    ...f,
    records: Number(f.records),
  ...
  try {
    ...
    if (slugs.length === 0) {
      ...
      for (const f of await inventario(db, [])) {
        console.log(
          `  ${f.slug.padEnd(28)} ${String(f.records).padStart(3)} fichas  ` +
    ...
    const rows = await inventario(db, slugs);
    const encontrados = new Set(rows.map((f) => f.slug));
    ...
    for (const f of rows) {
      ...
      console.log(
        `      ${f.records} fichas · ${f.staff} staff · ${f.cargos} cargos · ${f.asistencias} asistencias`,
      );
    }
    ...
    const names = huerfanos.rows ?? (huerfanos as unknown as { name: string }[]);

    console.log(`\n  gimnasios borrados: ${borrados.length}`);
    console.log(`  personas sin ficha borradas: ${names.length}`);
  } finally {
    await pool.end();
  }
}
